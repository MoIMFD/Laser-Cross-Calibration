...
class StagePositionOptimizer:
    ...
    def _merit_function(self, variables: np.ndarray) -> float:
        ...
        self.iteration_history.append({
            ...
            'intersections': [op.current_value for op in self.operands if op.operand_type.startswith('intersection')]
        })
    ...
    def optimize(self) -> OptimizationResult:
        ...
        if not self.variables:
            raise ValueError("No optimization variables defined. Call setup_stage_position_variables() first.")
        
        if not self.operands:
            raise ValueError("No optimization operands defined. Call add_operand() first.")
        ...
        try:
            ...
            final_intersections = [op.current_value for op in self.operands 
                                 if op.operand_type == "intersection_target" and op.current_value is not None]
            ...
            success = error_converged or (scipy_success and final_error < 10 * self.tolerance)
            ...
            target_intersections = [op.target_value for op in self.operands 
                                  if op.operand_type == "intersection_target"]
            target_intersection = target_intersections[0] if target_intersections else None
...
def find_stage_position_for_intersection(
    ...
    optimizer.add_operand("intersection_exists", True, weight=10.0)  # Ensure intersection exists
